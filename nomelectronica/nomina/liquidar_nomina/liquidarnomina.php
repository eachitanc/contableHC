...
try {
    $cmd = new PDO("$bd_driver:host=$bd_servidor;dbname=$bd_base;$charset", $bd_usuario, $bd_clave);
    $cmd->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_WARNING);
    $sql = "SELECT id_he_trab, id_empleado, seg_horas_ex_trab.id_he, fec_inicio, fec_fin, hora_inicio, hora_fin, cantidad_he, codigo, factor
            FROM
                seg_horas_ex_trab
...
try {
    $cmd = new PDO("$bd_driver:host=$bd_servidor;dbname=$bd_base;$charset", $bd_usuario, $bd_clave);
    $cmd->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_WARNING);
    $sql = "SELECT id_empleado FROM seg_empleado
            ORDER BY id_empleado DESC LIMIT 1;";
    $rs = $cmd->query($sql);
    $limite = $rs->fetch();
    $cmd = new PDO("$bd_driver:host=$bd_servidor;dbname=$bd_base;$charset", $bd_usuario, $bd_clave);
    $sql = "SELECT id_empleado, no_documento, CONCAT(nombre1, ' ', nombre2, ' ',apellido1, ' ', apellido2) AS nombre FROM seg_empleado
                WHERE  estado = '1'";
    $rs = $cmd->query($sql);
    $emple = $rs->fetchAll();
    $cmd = null;
...
try {
    ...
    $sql = "SELECT * 
            FROM 	
                    (SELECT seg_novedades_arl.id_empleado, seg_novedades_arl.id_arl, seg_novedades_arl.id_riesgo, salario_basico AS salario, cotizacion, seg_novedades_arl.fec_afiliacion
                    FROM
                    seg_novedades_arl
                    INNER JOIN seg_empleado 
                        ON (seg_novedades_arl.id_empleado = seg_empleado.id_empleado)
                    INNER JOIN seg_riesgos_laboral 
                        ON (seg_novedades_arl.id_riesgo = seg_riesgos_laboral.id_rlab)
                    INNER JOIN seg_novedades_arl AS seg_novedades_arl_1 
                        ON (seg_novedades_arl_1.id_empleado = seg_empleado.id_empleado) AND (seg_novedades_arl_1.id_riesgo = seg_riesgos_laboral.id_rlab)
...
for ($i = 0; $i <= $max; $i++) {
    if (isset($_POST['check' . $i])) {
        $cmd = new PDO("$bd_driver:host=$bd_servidor;dbname=$bd_base;$charset", $bd_usuario, $bd_clave);
        $sql = "SELECT * FROM seg_liq_salaro
        ...
        if (!isset($nomliq['id_empleado'])) {
            ...
            if (isset($_POST['numIdEmplDiaLab_' . $i])) {
                    ...
                    foreach ($horas as $h) {
                        if (intval($h['id_empleado']) === intval($i)) {
                            ...
                            try {
                                $cmd = new PDO("$bd_driver:host=$bd_servidor;dbname=$bd_base;$charset", $bd_usuario, $bd_clave);
                                $cmd->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_WARNING);
                                $sql = "INSERT INTO seg_liq_horex (id_he_lab, val_liq, mes_he, anio_he, fec_reg) VALUES (?, ?, ?, ?, ?)";
                                $sql = $cmd->prepare($sql);
                                ...
                                $res = $e->getCode() == 2002 ? 'Sin Conexión a Mysql (Error: 2002)' : 'Error: ' . $e->getCode();
                            }
                        }
                    }
                }
                ...
                foreach ($licencias as $lc) {
                    if (intval($i) === intval($lc['id_empleado'])) {
                        ...
                        if (intval($diflc) > 0) {
                            ...
                            $finlic = $fec_f;
                            $cmd = new PDO("$bd_driver:host=$bd_servidor;dbname=$bd_base;$charset", $bd_usuario, $bd_clave);
                            $sql = "UPDATE seg_licenciasmp SET  fec_inicio = ? WHERE id_licmp = ?";
                            $sql = $cmd->prepare($sql);
                        ...
                        try {
                            $cmd = new PDO("$bd_driver:host=$bd_servidor;dbname=$bd_base;$charset", $bd_usuario, $bd_clave);
                            $cmd->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_WARNING);
                            $sql = "INSERT INTO seg_liq_licmp (id_licmp, fec_inicio, fec_fin, dias_liqs, val_liq, val_dialc, mes_lic, anio_lic, fec_reg) VALUES (?, ?, ?, ?, ?,?, ?, ?, ?)";
                            $sql = $cmd->prepare($sql);
                ...
                foreach ($vacaciones as $vs) {
                    if (intval($i) === intval($vs['id_empleado'])) {
                        ...
                        if (intval($difvac) > 0) {
                            ...
                            $finvac = $fec_f;
                            $cmd = new PDO("$bd_driver:host=$bd_servidor;dbname=$bd_base;$charset", $bd_usuario, $bd_clave);
                            $sql = "UPDATE seg_vacaciones SET  fec_inicio = ? WHERE id_vac = ?";
                            $sql = $cmd->prepare($sql);
                            $sql->bindParam(1, $nextday, PDO::PARAM_STR);
                        ...
                        try {
                            $cmd = new PDO("$bd_driver:host=$bd_servidor;dbname=$bd_base;$charset", $bd_usuario, $bd_clave);
                            $cmd->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_WARNING);
                            $sql = "INSERT INTO seg_liq_vac (id_vac, fec_inicio, fec_fin, dias_liqs, val_liq, val_diavac, mes_vac, anio_vac, fec_reg) VALUES (?, ?, ?, ?, ?,?, ?, ?, ?)";
                            $sql = $cmd->prepare($sql);
                ...
                foreach ($incapacidades as $inc) {
                    if (intval($i) === intval($inc['id_empleado'])) {
                        ...
                        if (intval($dif) > 0) {
                            ...
                            $finincap = $fec_f;
                            $cmd = new PDO("$bd_driver:host=$bd_servidor;dbname=$bd_base;$charset", $bd_usuario, $bd_clave);
                            $sql = "UPDATE seg_incapacidad SET  fec_inicio = ? WHERE id_incapacidad = ?";
                            $sql = $cmd->prepare($sql);
                            $sql->bindParam(1, $nextday, PDO::PARAM_STR);
                        ...
                        try {
                            $cmd = new PDO("$bd_driver:host=$bd_servidor;dbname=$bd_base;$charset", $bd_usuario, $bd_clave);
                            $cmd->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_WARNING);
                            $sql = "INSERT INTO seg_liq_incap (id_incapacidad, fec_inicio, fec_fin, dias_liq, pago_empresa, pago_eps, pago_arl, mes, anios, fec_reg) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
                            $sql = $cmd->prepare($sql);
                ...
                try {
                    $cmd = new PDO("$bd_driver:host=$bd_servidor;dbname=$bd_base;$charset", $bd_usuario, $bd_clave);
                    $cmd->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_WARNING);
                    $sql = "INSERT INTO seg_liq_dias_lab (id_empleado, cant_dias, mes, anio, fec_reg) VALUES (?, ?, ?, ?, ?)";
                    $sql = $cmd->prepare($sql);
                ...
                try {
                    $cmd = new PDO("$bd_driver:host=$bd_servidor;dbname=$bd_base;$charset", $bd_usuario, $bd_clave);
                    $cmd->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_WARNING);
                    $sql = "INSERT INTO seg_liq_parafiscales (id_empleado, val_sena, val_icbf, val_comfam, mes_pfis, anio_pfis, fec_reg) VALUES (?, ?, ?, ?, ?, ?, ?)";
                    $sql = $cmd->prepare($sql);
                ...
                try {
                    $cmd = new PDO("$bd_driver:host=$bd_servidor;dbname=$bd_base;$charset", $bd_usuario, $bd_clave);
                    $cmd->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_WARNING);
                    $sql = "INSERT INTO seg_liq_segsocial_empdo (id_empleado, aporte_salud_emp, aporte_pension_emp, aporte_solidaridad_pensional, aporte_salud_empresa, aporte_pension_empresa, aporte_rieslab, mes, anio, fec_reg) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
                    $sql = $cmd->prepare($sql);
                    $sql->bindParam(1, $i, PDO::PARAM_INT);
                ...
                try {
                    $cmd = new PDO("$bd_driver:host=$bd_servidor;dbname=$bd_base;$charset", $bd_usuario, $bd_clave);
                    $cmd->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_WARNING);
                    $sql = "INSERT INTO seg_liq_dlab_auxt (id_empleado, dias_liq, val_liq_dias, val_liq_auxt, mes_liq, anio_liq, fec_reg) VALUES (?, ?, ?, ?, ?, ?, ?)";
                    $sql = $cmd->prepare($sql);
                    $sql->bindParam(1, $empleado, PDO::PARAM_INT);
                ...
                try {
                    $cmd = new PDO("$bd_driver:host=$bd_servidor;dbname=$bd_base;$charset", $bd_usuario, $bd_clave);
                    $cmd->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_WARNING);
                    $sql = "INSERT INTO seg_liq_prestaciones_sociales (id_empleado, val_vacacion, val_cesantia, val_interes_cesantia, val_prima, mes_prestaciones, anio_prestaciones, fec_reg) VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
                    $sql = $cmd->prepare($sql);
                    $sql->bindParam(1, $empleado, PDO::PARAM_INT);
                ...
                if (isset($libranza)) {
                    foreach ($libranza as $libranza) {
                        ...
                        try {
                            $cmd = new PDO("$bd_driver:host=$bd_servidor;dbname=$bd_base;$charset", $bd_usuario, $bd_clave);
                            $cmd->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_WARNING);
                            $sql = "INSERT INTO seg_liq_libranza (id_libranza, val_mes_lib, mes_lib, anio_lib, fec_reg) VALUES (?, ?, ?, ?, ?)";
                            $sql = $cmd->prepare($sql);
                            ...
                            if ($idembargo !== '') {
                                ...
                                if ($dctoemb !== '0') {
                                    try {
                                        $cmd = new PDO("$bd_driver:host=$bd_servidor;dbname=$bd_base;$charset", $bd_usuario, $bd_clave);
                                        $cmd->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_WARNING);
                                        $sql = "INSERT INTO seg_liq_embargo (id_embargo, val_mes_embargo, mes_embargo, anio_embargo, fec_reg) VALUES (?, ?, ?, ?, ?)";
                                        $sql = $cmd->prepare($sql);
                ...
                if ($idcuotsind !== '0') {
                    try {
                        $cmd = new PDO("$bd_driver:host=$bd_servidor;dbname=$bd_base;$charset", $bd_usuario, $bd_clave);
                        $cmd->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_WARNING);
                        $sql = "INSERT INTO seg_liq_sindicato_aportes (id_cuota_sindical, val_aporte, mes_aporte, anio_aporte, fec_reg) VALUES (?, ?, ?, ?, ?)";
                        $sql = $cmd->prepare($sql);
                ...
                try {
                    $cmd = new PDO("$bd_driver:host=$bd_servidor;dbname=$bd_base;$charset", $bd_usuario, $bd_clave);
                    $cmd->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_WARNING);
                    $sql = "INSERT INTO seg_liq_salaro (id_empleado, val_liq, forma_pago, metodo_pago, mes, anio, fec_reg) VALUES (?, ?, ?, ?, ?, ?, ?)";
                    $sql = $cmd->prepare($sql);
                    $sql->bindParam(1, $i, PDO::PARAM_INT);
...
function desactivaremb($a, $d)
{
    ...
    try {
        ...
        $cmd = new PDO("$bd_driver:host=$bd_servidor;dbname=$bd_base;$charset", $bd_usuario, $bd_clave);
        $cmd->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_WARNING);
        $sql = "UPDATE seg_embargos SET estado = ? WHERE id_embargo = ?";
        $sql = $cmd->prepare($sql);
        ...
        if ($sql->rowCount() > 0) {
            $cmd = new PDO("$bd_driver:host=$bd_servidor;dbname=$bd_base;$charset", $bd_usuario, $bd_clave);
            $cmd->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_WARNING);
            $sql = "UPDATE seg_embargos SET  fec_act = ? WHERE id_embargo = ?";
            $sql = $cmd->prepare($sql);
...
function desactivarlib($a, $d)
{
    ...
    try {
        ...
        $cmd = new PDO("$bd_driver:host=$bd_servidor;dbname=$bd_base;$charset", $bd_usuario, $bd_clave);
        $cmd->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_WARNING);
        $sql = "UPDATE seg_libranzas SET estado = ? WHERE id_libranza = ?";
        $sql = $cmd->prepare($sql);
        $sql->bindParam(1, $estado, PDO::PARAM_STR);
        ...
        if ($sql->rowCount() > 0) {
            $cmd = new PDO("$bd_driver:host=$bd_servidor;dbname=$bd_base;$charset", $bd_usuario, $bd_clave);
            $cmd->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_WARNING);
            $sql = "UPDATE seg_libranzas SET  fec_act = ? WHERE id_libranza = ?";
            $sql = $cmd->prepare($sql);
            $sql->bindValue(1, $date->format('Y-m-d H:i:s'));
            $sql->bindParam(2, $idlibr, PDO::PARAM_INT);
            $sql->execute();
