...
try {
    ...
    $rs = $cmd->query($sql);
    $limite = $rs->fetch();
    $sql = "SELECT id_empleado,no_documento, CONCAT(nombre1, ' ', nombre2, ' ', apellido1, ' ', apellido2) as nombre 
            FROM seg_empleado";
    $rs = $cmd->query($sql);
    ...
    if (isset($_POST['check' . $i])) {
        ...
        if (false !== $key) {
            ...
            try {
                $cmd = new PDO("$bd_driver:host=$bd_servidor;dbname=$bd_base;$charset", $bd_usuario, $bd_clave);
                $cmd->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_WARNING);
                $sql = "INSERT INTO seg_liq_prima (id_empleado, cant_dias, val_liq_ps, periodo, anio, fec_reg) VALUES (?, ?, ?, ?, ?, ?)";
                $sql = $cmd->prepare($sql);
