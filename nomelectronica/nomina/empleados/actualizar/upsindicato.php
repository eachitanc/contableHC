...
try {
    ...
    $sql = "UPDATE seg_cuota_sindical SET id_sindicato = ?, porcentaje_cuota = ?, fec_inicio = ?, fec_fin = ? WHERE id_cuota_sindical = ?";
    $sql = $cmd->prepare($sql);
    ...
    $sql->bindParam(5, $idsind, PDO::PARAM_INT);
    $sql->execute();
    if ($sql->rowCount() > 0) {
        $updata = 1;
    } else {
    ...
    if ( $updata > 0) {
        ...
        $sql = "UPDATE seg_cuota_sindical SET  fec_act = ? WHERE id_cuota_sindical = ?";
        $sql = $cmd->prepare($sql);
