...
try {
    ...
    $sql = "UPDATE seg_novedades_afp SET id_afp = ?, fec_afiliacion = ?, fec_retiro = ? WHERE id_novafp = ?";
    $sql = $cmd->prepare($sql);
    ...
    if ($updata > 0) {
        $cmd = new PDO("$bd_driver:host=$bd_servidor;dbname=$bd_base;$charset", $bd_usuario, $bd_clave);
        $cmd->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_SILENT);
        $sql = "UPDATE seg_novedades_afp SET  fec_act = ? WHERE id_novafp = ?";
        $sql = $cmd->prepare($sql);
