...
try {
    ...
    $sql = "UPDATE seg_licenciasmp SET fec_inicio = ?, fec_fin = ?,  dias_inactivo = ?, dias_habiles = ? WHERE id_licmp = ?";
    $sql = $cmd->prepare($sql);
    ...
    if ($updata > 0) {
        $cmd = new PDO("$bd_driver:host=$bd_servidor;dbname=$bd_base;$charset", $bd_usuario, $bd_clave);
        $cmd->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_SILENT);
        $sql = "UPDATE seg_licenciasmp SET  fec_act = ? WHERE id_licmp = ?";
        $sql = $cmd->prepare($sql);
