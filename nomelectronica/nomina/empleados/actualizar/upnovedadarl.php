...
try {
    ...
    $sql = "UPDATE seg_novedades_arl SET id_arl = ?, id_riesgo = ?, fec_afiliacion= ?, fec_retiro = ? WHERE id_novarl = ?";
    $sql = $cmd->prepare($sql);
    ...
    if ($updata > 0) {
        $cmd = new PDO("$bd_driver:host=$bd_servidor;dbname=$bd_base;$charset", $bd_usuario, $bd_clave);
        $cmd->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_SILENT);
        $sql = "UPDATE seg_novedades_arl SET  fec_act = ? WHERE id_novarl = ?";
        $sql = $cmd->prepare($sql);
