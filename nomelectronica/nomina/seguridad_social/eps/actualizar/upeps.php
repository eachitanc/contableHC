...
$ide = $_POST['numIdEps'];
$n = $_POST['txtNitUpEps'];
$nom = $_POST['txtNomUpEps'];
$tel = $_POST['txtTelUpEps'];
$cor = $_POST['mailUpeps'];
$dv = calcularDV($n);
...
try {
    ...
    $sql = "UPDATE seg_epss SET nombre_eps = ?, nit = ?, digito_verific = ?, telefono = ?, correo = ? WHERE id_eps = ?";
    $sql = $cmd->prepare($sql);
    ...
    if ($updata > 0) {
        $cmd = new PDO("$bd_driver:host=$bd_servidor;dbname=$bd_base;$charset", $bd_usuario, $bd_clave);
        $cmd->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_SILENT);
        $sql = "UPDATE seg_epss SET fec_act = ? WHERE id_eps = ?";
        $sql = $cmd->prepare($sql);
