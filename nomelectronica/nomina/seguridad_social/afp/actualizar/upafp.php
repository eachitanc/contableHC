...
$ide = $_POST['numIdAfp'];
$n = $_POST['txtNitUpAfp'];
$nom = $_POST['txtNomUpAfp'];
$tel = $_POST['txtTelUpAfp'];
$cor = $_POST['mailUpafp'];
$dv = calcularDV($n);
...
try {
    ...
    $sql = "UPDATE seg_afp SET nombre_afp = ?, nit_afp = ?, dig_verf = ?, telefono = ?, correo = ? WHERE id_afp = ?";
    $sql = $cmd->prepare($sql);
    ...
    if ($updata > 0) {
        $cmd = new PDO("$bd_driver:host=$bd_servidor;dbname=$bd_base;$charset", $bd_usuario, $bd_clave);
        $sql = "UPDATE seg_afp SET fec_act = ? WHERE id_afp = ?";
        $sql = $cmd->prepare($sql);
