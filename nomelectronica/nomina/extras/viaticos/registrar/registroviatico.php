...
try {
    ...
    $sql = "SELECT *
            ...
            WHERE vigencia = '$vig' AND seg_salarios_basico.id_empleado = '$idemviat'";
    $rs = $cmd->query($sql);
    $empleado = $rs->fetch();
    $cmd = null;
...
try {
    $cmd = new PDO("$bd_driver:host=$bd_servidor;dbname=$bd_base;$charset", $bd_usuario, $bd_clave);
    $cmd->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_WARNING);
    $sql = "SELECT id_vigencia FROM con_vigencias WHERE anio = '$vig'";
    $rs = $cmd->query($sql);
    $vigen = $rs->fetch();
    $idvig = $vigen['id_vigencia'];
    $sql = "SELECT val_viatico_dia FROM seg_rango_viaticos WHERE val_min <= '$sal' AND val_max >= '$sal' AND vigencia = '$idvig'";
    $rs = $cmd->query($sql);
    $valviat = $rs->fetch();
