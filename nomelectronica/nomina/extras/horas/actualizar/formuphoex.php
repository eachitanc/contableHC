...
$idhetrab = $_POST['idUpHoexs'];
$m = $_POST['valmesreghe'];
$idreg = $_POST['validreghe'];
try {
    $cmd = new PDO("$bd_driver:host=$bd_servidor;dbname=$bd_base;$charset", $bd_usuario, $bd_clave);
    $sql = "SELECT id_he_trab,seg_horas_ex_trab.id_he,desc_he, fec_inicio, fec_fin, hora_inicio, hora_fin, cantidad_he FROM seg_horas_ex_trab
...
try {
    $cmd = new PDO("$bd_driver:host=$bd_servidor;dbname=$bd_base;$charset", $bd_usuario, $bd_clave);
    $sql = "SELECT * FROM seg_tipo_horaex";
    $rs = $cmd->query($sql);
    $fila = $rs->fetchAll();
    $cmd = null;
} catch (PDOException $e) {
                            ...
                            <form id="formupHoex">
                                ...
                                <div class="form-row">
                                    ...
                                    <div class="form-group col-md-4">
                                        ...
                                        <div class="form-control form-control-sm" id="CantHeup">
                                            <?php echo $obj['cantidad_he'] ?>
                                            <input type="number" id="numCantHeup" name="numCantHe" value="<?php echo $obj['cantidad_he'] ?>" hidden>
                                        </div>
                                    </div>
                                </div>
                                <button class="btn btn-primary btn-sm" id="btnUpHoex"> Actualizar</button>
                                <a type="button" class="btn btn-danger btn-sm" href="../listhoraextra.php"> Cancelar</a>
        ...
        <div class="modal fade" id="divModalErrorDelHex" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle" aria-hidden="true">
            <div class="modal-dialog modal-dialog-centered" role="document">
                <div class="modal-content">
                    ...
                    <div class="modal-footer">
                        <button type="button" class="btn btn-danger btn-sm" id="btnXErrorDel">Cancelar</button>
                    </div>
                </div>
            </div>
        </div>
