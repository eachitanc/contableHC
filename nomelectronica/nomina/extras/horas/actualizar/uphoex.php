...
try {
    ...
    $sql = "UPDATE seg_horas_ex_trab SET id_he = ?, fec_inicio = ?, fec_fin = ?, hora_inicio = ?, hora_fin = ?, cantidad_he = ? WHERE id_he_trab = ?";
    $sql = $cmd->prepare($sql);
    ...
    if ($updata > 0) {
        $cmd = new PDO("$bd_driver:host=$bd_servidor;dbname=$bd_base;$charset", $bd_usuario, $bd_clave);
        $cmd->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_SILENT);
        $sql = "UPDATE seg_horas_ex_trab SET fec_actu = ? WHERE id_he_trab = ?";
        $sql = $cmd->prepare($sql);
