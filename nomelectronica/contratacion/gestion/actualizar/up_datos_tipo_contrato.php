...
$idtcontrato = $_POST['idTipoContrato'];
...
try {
    ...
    $sql = "UPDATE seg_tipo_contrata SET id_tipo_compra = ?, tipo_contrato = ? WHERE id_tipo = ?";
    $sql = $cmd->prepare($sql);
    ...
    if (!($sql->execute())) {
        ...
        if ($cambio > 0) {
            $cmd = new PDO("$bd_driver:host=$bd_servidor;dbname=$bd_base;$charset", $bd_usuario, $bd_clave);
            $cmd->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_SILENT);
            $sql = "UPDATE seg_tipo_contrata SET  id_user_act = ? ,fec_act = ? WHERE id_tipo = ?";
            $sql = $cmd->prepare($sql);
