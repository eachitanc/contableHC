...
//****
//****
$resultax=mysql_query("select SUM(ppto_aprob) AS TOTAL from car_ppto_gas WHERE (ano between '$fecha_ini' and '$fecha_fin' )  and tip_dato ='D' and cod_pptal LIKE '$cod%'",$link) or die (mysql_error());
$rowx=mysql_fetch_row($resultax);
$inicial=$rowx[0]; 
//****
$resulta=mysql_query("select SUM(valor_adi) AS TOTAL from adi_ppto_gas WHERE (fecha_adi between '$fecha_ini' and '$fecha_fin' ) and cod_pptal LIKE '$cod%'",$link) or die (mysql_error());
$row=mysql_fetch_row($resulta);
$total_adi=$row[0]; 

$resulta2=mysql_query("select SUM(valor_adi) AS TOTAL from red_ppto_gas WHERE (fecha_adi between '$fecha_ini' and '$fecha_fin' ) and cod_pptal LIKE '$cod%'",$link) or die (mysql_error());
$row2=mysql_fetch_row($resulta2);
$total_red=$row2[0];

$resulta3=mysql_query("select SUM(valor_adi) AS TOTAL from creditos WHERE (fecha_adi between '$fecha_ini' and '$fecha_fin' ) and cod_pptal LIKE '$cod%'",$link) or die (mysql_error());
$row3=mysql_fetch_row($resulta3);
$total_cre=$row3[0];

$resulta4=mysql_query("select SUM(valor_adi) AS TOTAL from contracreditos WHERE (fecha_adi between '$fecha_ini' and '$fecha_fin' )  and cod_pptal LIKE '$cod%'",$link) or die (mysql_error());
$row4=mysql_fetch_row($resulta4);
$total_ccre=$row4[0];

$definitivo = $inicial + $total_adi - $total_red + $total_cre - $total_ccre;
$resulta5=mysql_query("select SUM(valor) AS TOTAL from cdpp WHERE (fecha_reg between '$fecha_ini' and '$fecha_fin' )  and cuenta LIKE '$cod%'",$link) or die (mysql_error());
...
$ene_ini = $anno."/01/01";
$ene_fin = $anno."/01/31";
$sqlceva = mysql_query("SELECT SUM(enero) AS enero FROM pac_gastos where  cod_pptal LIKE '$cod%' ",$link) or die (mysql_error());
$row8=mysql_fetch_row($sqlceva);
$enero=$row8[0];
// febrero
$feb_ini = $anno."/02/01";
$feb_fin = $anno."/02/29";
$sqlceva = mysql_query("SELECT SUM(febrero) AS enero FROM pac_gastos where  cod_pptal LIKE '$cod%' ",$link) or die (mysql_error());
$row8=mysql_fetch_row($sqlceva);
...
$mar_ini = $anno."/03/01";
$mar_fin = $anno."/03/31";
$sqlceva = mysql_query("SELECT SUM(marzo) AS enero FROM pac_gastos where  cod_pptal LIKE '$cod%' ",$link) or die (mysql_error());
$row8=mysql_fetch_row($sqlceva);
...
$abr_ini = $anno."/04/01";
$abr_fin = $anno."/04/31";
$sqlceva = mysql_query("SELECT SUM(abril) AS enero FROM pac_gastos where  cod_pptal LIKE '$cod%' ",$link) or die (mysql_error());
$row8=mysql_fetch_row($sqlceva);
...
$may_ini = $anno."/05/01";
$may_fin = $anno."/05/31";
$sqlceva = mysql_query("SELECT SUM(mayo) AS enero FROM pac_gastos where  cod_pptal LIKE '$cod%' ",$link) or die (mysql_error());
$row8=mysql_fetch_row($sqlceva);
...
$jun_ini = $anno."/06/01";
$jun_fin = $anno."/06/30";
$sqlceva = mysql_query("SELECT SUM(junio) AS enero FROM pac_gastos where  cod_pptal LIKE '$cod%' ",$link) or die (mysql_error());
$row8=mysql_fetch_row($sqlceva);
$junio=$row8[0];
// Julio
$jul_ini = $anno."/07/01";
$jul_fin = $anno."/07/31";
$sqlceva = mysql_query("SELECT SUM(julio) AS enero FROM pac_gastos where  cod_pptal LIKE '$cod%' ",$link) or die (mysql_error());
$row8=mysql_fetch_row($sqlceva);
$julio=$row8[0];
// Agosto
$ago_ini = $anno."/08/01";
$ago_fin = $anno."/08/31";
$sqlceva = mysql_query("SELECT SUM(agosto) AS enero FROM pac_gastos where  cod_pptal LIKE '$cod%' ",$link) or die (mysql_error());
$row8=mysql_fetch_row($sqlceva);
$agosto=$row8[0];
// Septiembre
$sep_ini = $anno."/09/01";
$sep_fin = $anno."/09/30";
$sqlceva = mysql_query("SELECT SUM(septiembre) AS enero FROM pac_gastos where  cod_pptal LIKE '$cod%' ",$link) or die (mysql_error());
$row8=mysql_fetch_row($sqlceva);
$septiembre=$row8[0];
// octubre
$oct_ini = $anno."/10/01";
$oct_fin = $anno."/10/31";
$sqlceva = mysql_query("SELECT SUM(octubre) AS enero FROM pac_gastos where  cod_pptal LIKE '$cod%' ",$link) or die (mysql_error());
$row8=mysql_fetch_row($sqlceva);
$octubre=$row8[0];
// Noviembre
$nov_ini = $anno."/11/01";
$nov_fin = $anno."/11/30";
$sqlceva = mysql_query("SELECT SUM(noviembre) AS enero FROM pac_gastos where  cod_pptal LIKE '$cod%' ",$link) or die (mysql_error());
$row8=mysql_fetch_row($sqlceva);
$noviembre=$row8[0];
// Diciembre
$dic_ini = $anno."/12/01";
$dic_fin = $anno."/12/31";
$sqlceva = mysql_query("SELECT SUM(diciembre) AS enero FROM pac_gastos where  cod_pptal LIKE '$cod%' ",$link) or die (mysql_error());
$row8=mysql_fetch_row($sqlceva);
