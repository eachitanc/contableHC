...
$resultax=mysql_query("select SUM(ppto_aprob) AS TOTAL from car_ppto_ing WHERE  tip_dato ='D' and cod_pptal LIKE '$cod%'",$cx) or die (mysql_error());
$rowx=mysql_fetch_row($resultax);
$inicial=$rowx[0];
//****

$resulta=mysql_query("select SUM(valor_adi) AS TOTAL from adi_ppto_ing WHERE  cod_pptal LIKE '$cod%'",$cx) or die (mysql_error());
$row=mysql_fetch_row($resulta);
$total_adi=$row[0]; 

$resulta2=mysql_query("select SUM(valor_adi) AS TOTAL from red_ppto_ing WHERE  cod_pptal LIKE '$cod%'",$cx) or die (mysql_error());
$row2=mysql_fetch_row($resulta2);
$total_red=$row2[0];

$resulta9=mysql_query("select SUM(valor_adi) AS TOTAL from contracreditos_ing WHERE  cod_pptal LIKE '$cod%'",$cx) or die (mysql_error());
$row9=mysql_fetch_row($resulta9);
$total_cotracred=$row9[0];

$resulta8=mysql_query("select SUM(valor_adi) AS TOTAL from creditos_ing WHERE cod_pptal LIKE '$cod%'",$cx) or die (mysql_error());
...
if ($rw["proc_rec"] == 'R') {$fodo2 = 'bgcolor=#FFCC00'; }else{ $fodo2 = '';}
...
</tr>", $rw["cod_pptal"], $rw["ano"],$situa,$proc,$rw["vigencia"], $rw["cod_pptal"], $rw["cod_pptal"], 
