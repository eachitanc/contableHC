...
?>		  
		    <input type="hidden" name="consecutivo" value="<? $consecutivo = $_GET['var']; printf("$consecutivo"); ?>" />
			<input type="hidden" name="cuenta" value="<? $cuenta = $_GET['cta_cdpp']; printf("$cuenta"); ?>" />
			<input type="hidden" name="nom_rubro" value="<? $nom_rubro; printf("$nom_rubro"); ?>" />
			<input type="hidden" name="cdpp" value="<? $cdpp = $_GET['id_manu_cdpp']; printf("$cdpp"); ?>" />
			<input type="hidden" name="vr_obligado" value="<? printf("$vr_obligado"); ?>" />
			<input type="hidden" name="liq1" value="<? $liq1='SI'; printf("$liq1"); ?>" />
			<input type="hidden" name="liq2" value="<? $liq2=$_GET['vr_x_reg']* (-1) ; printf("$liq2"); ?>" />
			
            <input name="fecha_reg" type="text" class="Estilo4" id="fecha_reg" value="<?php $bb=date("Y/m/d"); printf($bb); ?>" size="12" />
      ...
      <tr>
        ...
        <td colspan="2"><div style="padding-left:5px; padding-top:5px; padding-right:5px; padding-bottom:5px;">
          <div align="center">
            <input name="valor" type="text" class="required max Estilo4" id="valor" onkeypress="return validar(event)" style="text-align:center" value="<? printf("%s",$valor);?>" max="<? printf("%s",$valor);?>" />
         </div>
        </div></td>
      </tr>
      <tr>
...
<br />
<br />
<a href="hisreip.php?consecutivo=<? printf($consecutivoa); ?>" target="_parent">VOLVER</a>
</center>
</TD>
</TR>
</TABLE>
...
<form name="form2">
<input type="button" value="Atrás" onclick="history.back()" class = Estilo4>
</form>
</center>
<?
}
