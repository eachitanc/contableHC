...
<style type="text/css">
<!--
.Estilo2 {font-size: 10px; font-family: Verdana, Geneva, Arial, Helvetica, sans-serif; font-weight:bolder; color: #333333;}
...
if ($mes == $a.'/02/28' && $bis ==1) { $fecha_fin=$a."/02/29"; $fecha_marca=$a."/02/29";}else{$fecha_fin=$mes;}  
echo "
<center>
...
while ($rw1 = mysql_fetch_array($re))
	...
	$re23 = mysql_db_query($database, $s23, $cx);
	while($rw23 = mysql_fetch_array($re23)) 
	{
	...
	while($rw = mysql_fetch_array($re2)) 
	   {
				...
				, $rw["nom_rubro"],number_format($saldo,2,'.',','), number_format($debitos_t,2,'.',','),number_format($creditos_t,2,'.',','),number_format($diferencia,2,'.',',')); 
		$diferencia='';
		 ...
		 if ($saldo != 0  || $saldo == 0)
			...
			echo "<form name='concil_$cont2' id='concil_$cont2' method='post' action='conciliaciones2.php' ";
			  printf("
						...
						,$rw1['nom_rubro'],number_format($saldo,2,',','.'), number_format($tot_debitos,2,',','.'),number_format('0.00',2,',','.'),number_format($diferencia,2,',','.'));
					echo "</form>";
