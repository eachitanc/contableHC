...
<?
			...
			$rcon=mysql_fetch_array($rescon);
			$cont=0; $co=0;
			for($y=1;$y<=15;$y++) {if($rcon['pgcp'.$y]) $cont++;} // echo $cont;
			
			$sqcon2="select * from cecp where id_auto_cecp='$id_cecp'";
			$rescon2=mysql_db_query($database, $sqcon2, $connectionxx);
			$rcon2=mysql_fetch_array($rescon2);			
			for($z=1;$z<=15;$z++) {if($rcon2['vr_deb_'.$z]>0) $co++; else break;} //echo $co;
			
			$sqcu="select * from cecp_cuenta  where id_auto_cecp='$id_cecp'";
			$rescu=mysql_db_query($database, $sqcu, $connectionxx);
			$s=0;
			while($rcu=mysql_fetch_array($rescu))
			{
				$arr[$s] = $rcu['cuenta'];
				$s++;
			}
...
$link=mysql_connect($server,$dbuser,$dbpass);
		...
		for ($k=1;$k<=20;$k++)
		{	
			$cuenta_cxp = $arr[$k-1]; //echo $cuenta_cxp; 
