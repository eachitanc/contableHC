...
$cod=$rw["cod_pptal"];
$opc=$rw["opc1"];
if ($opc == 'MANTENIMIENTO') {$ver ="bgcolor='#FFCC00'";} else {$ver ="";}
//****
//****
$resultax=mysql_query("select SUM(ppto_aprob) AS TOTAL from car_ppto_gas WHERE  tip_dato ='D' and cod_pptal LIKE '$cod%'",$cx) or die (mysql_error());
$rowx=mysql_fetch_row($resultax);
$inicial=$rowx[0]; 
//****
$resulta=mysql_query("select SUM(valor_adi) AS TOTAL from adi_ppto_gas WHERE cod_pptal LIKE '$cod%'",$cx) or die (mysql_error());
$row=mysql_fetch_row($resulta);
$total_adi=$row[0]; 

$resulta2=mysql_query("select SUM(valor_adi) AS TOTAL from red_ppto_gas WHERE  cod_pptal LIKE '$cod%'",$cx) or die (mysql_error());
$row2=mysql_fetch_row($resulta2);
$total_red=$row2[0];

$resulta3=mysql_query("select SUM(valor_adi) AS TOTAL from creditos WHERE  cod_pptal LIKE '$cod%'",$cx) or die (mysql_error());
$row3=mysql_fetch_row($resulta3);
$total_cre=$row3[0];

$resulta4=mysql_query("select SUM(valor_adi) AS TOTAL from contracreditos WHERE  cod_pptal LIKE '$cod%'",$cx) or die (mysql_error());
...
if ($rw['proc_rec'] =='P') {$rec = 'PROPIO'; $ver4 ='';}	 
if ($rw['proc_rec'] =='R') {$rec = 'REGALIAS'; $ver4 = 'bgcolor=#FFCC33';}	 
if ($rw['proc_rec'] =='S') {$rec = 'SGP';}	 
if ($rw['proc_rec'] =='C') {$rec = 'COL HUMANITARIA';}	 
if ($rw['proc_rec'] =='O') {$rec = 'OTROS';}	   

if ($rw["vg_futura"]=='SI') $ver5 = 'bgcolor=#00CCFF';	   
$vr_df=$definitivo;   
printf("
<span class='Estilo4'>
<tr>
...
</tr>", $rw["cod_pptal"], $rw["ano"], $rw["situacion"],$rw["vigencia"], $rec,$rw["vg_futura"],$rw["cod_pptal"], $rw["cod_pptal"], 
		$rw["nom_rubro"], $rw["tip_dato"], number_format($vr_df,2,'.',','), 
		$rw["cod_pptal"], $rw["pac"],$rw["cod_pptal"]); 
