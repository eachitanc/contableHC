...
// consulto si el padre es MAYOR o DETALLE para evitar papas DETALLE con hijos DETALLE
	  $consulta=mysql_query("select tip_dato from car_ppto_gas where cod_pptal ='$padre' and id_emp ='$idxx'",$connection);
...
// consulto si el padre es MAYOR o DETALLE para evitar papas DETALLE con hijos DETALLE
      ...
      //-----------------------------------------------------------		
			...
			$resulta=mysql_query("select SUM(definitivo) AS TOTAL from car_ppto_gas WHERE padre = '2' AND nivel = '2' AND 						
			...
			$link=mysql_connect($server,$dbuser,$dbpass);
			$resulta=mysql_query("select SUM(definitivo) AS TOTAL from car_ppto_gas WHERE padre = '0' AND nivel = '2' AND 						
			id_emp='$idxx'",$link)or die(mysql_error());
			$row=mysql_fetch_row($resulta);
			$total=$row[0]; 
			$nuevo_total = $total;
...
// consulto si el padre es MAYOR o DETALLE para evitar papas DETALLE con hijos DETALLE
	  ...
	  //--------- SUMO TODOS LOS REGISTROS DE NIVEL 3 -----
			$link=mysql_connect($server,$dbuser,$dbpass);
			$resulta=mysql_query("select SUM(definitivo) AS TOTAL from car_ppto_gas WHERE padre='$padre' AND nivel=$nivel 
...
// consulto si el padre es MAYOR o DETALLE para evitar papas DETALLE con hijos DETALLE
	  ...
	  //--------- SUMO TODOS LOS REGISTROS DE NIVEL 4 -----
			$link=mysql_connect($server,$dbuser,$dbpass);
			$resulta=mysql_query("select SUM(definitivo) AS TOTAL from car_ppto_gas WHERE padre='$padre' AND nivel=$nivel AND 
			id_emp='$idxx'",$link)or die(mysql_error());
			$row=mysql_fetch_row($resulta);
			$total=$row[0]; 
			$nuevo_total = $total;
	  ...
	  //--------- SUMO TODOS LOS REGISTROS DE NIVEL 3 -----
			$link=mysql_connect($server,$dbuser,$dbpass);
			$resulta=mysql_query("select SUM(definitivo) AS TOTAL from car_ppto_gas WHERE 
...
// consulto si el padre es MAYOR o DETALLE para evitar papas DETALLE con hijos DETALLE
	  ...
	  //--------- SUMO TODOS LOS REGISTROS DE NIVEL 4 -----
			$link=mysql_connect($server,$dbuser,$dbpass);
			$resulta=mysql_query("select SUM(definitivo) AS TOTAL from car_ppto_gas WHERE padre='$c3' AND nivel=$niv4 AND 
			id_emp='$idxx'",$link)or die(mysql_error());
			$row=mysql_fetch_row($resulta);
			$total=$row[0]; 
			$nuevo_total = $total;
...
// consulto si el padre es MAYOR o DETALLE para evitar papas DETALLE con hijos DETALLE
	  ...
	  //--------- SUMO TODOS LOS REGISTROS DE NIVEL 6 -----
			$link=mysql_connect($server,$dbuser,$dbpass);
			$resulta=mysql_query("select SUM(definitivo) AS TOTAL from car_ppto_gas 
	  ...
	  //--------- SUMO TODOS LOS REGISTROS DE NIVEL 3 -----
			...
			$link=mysql_connect($server,$dbuser,$dbpass);
			$resulta=mysql_query("select SUM(definitivo) AS TOTAL from car_ppto_gas 
			WHERE padre = '2' AND nivel = '2' AND id_emp='$idxx'",$link)or die(mysql_error());
			$row=mysql_fetch_row($resulta);
			$total=$row[0]; 
			$nuevo_total = $total;
			//--------- ACTUALIZAR PADRE NIVEL 1 CUANDO EMPIEZA CON 1 -----
